  ...
  return (
    <Box>
      <Flex mb={4} alignItems="center">
        ...
        <Button ml={4} onClick={handlePass} disabled={validMoves.length > 0}>
          Pass
        </Button>
      </Flex>
      <Grid gridTemplateColumns={`repeat(${BOARD_SIZE}, 1fr)`} gridTemplateRows={`repeat(${BOARD_SIZE}, 1fr)`} gap={1}>
        {board.map((row, rowIndex) => row.map((cell, colIndex) => <Box key={`${rowIndex}-${colIndex}`} bg={cell === 1 ? "black" : cell === 2 ? "white" : "green.500"} borderRadius="md" display="flex" justifyContent="center" alignItems="center" onClick={() => placePiece(rowIndex, colIndex)} cursor={isValidMove(board, rowIndex, colIndex, currentPlayer) ? "pointer" : "default"} />))}
      </Grid>
    </Box>
  );
};
